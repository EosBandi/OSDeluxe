    ...
    while (Serial1.available() > 0)
    {
        ...
        {   
            ...
            case MAVLINK_MSG_ID_ATTITUDE:
            {
                ...
                g.pitch = ToDeg(mavlink_msg_attitude_get_pitch(&msg));
                g.roll = ToDeg(mavlink_msg_attitude_get_roll(&msg));
            ...
            case MAVLINK_MSG_ID_RC_CHANNELS:
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    if (g.rcin[osd.ctr_ch[i]] < RC_STATE1)
                        g.ctr_state[i] = 0;
                    else if (g.rcin[osd.ctr_ch[i]] < RC_STATE2)
                        g.ctr_state[i] = 1;
                    else if (g.rcin[osd.ctr_ch[i]] < RC_STATE3)
                        g.ctr_state[i] = 2;
                    else if (g.rcin[osd.ctr_ch[i]] < RC_STATE4)
                        g.ctr_state[i] = 3;
                    else if (g.rcin[osd.ctr_ch[i]] < RC_STATE5)
                        g.ctr_state[i] = 4;
                    else
                        g.ctr_state[i] = 5;
                }
            ...
            case MAVLINK_MSG_ID_EKF_STATUS_REPORT:
            {
                ...
                if (g.ekfvel > 50 || g.ekfposh > 50 || g.ekfposv > 50 || g.ekfcompass > 50 || g.ekfterrain > 50) osd.stat.ekf_status = STATUS_WARNING;

                if (g.ekfvel > 80 || g.ekfposh > 80 || g.ekfposv > 80 || g.ekfcompass > 80 || g.ekfterrain > 80) osd.stat.ekf_status = STATUS_CRITICAL;
            }
            break;
            ...
            case MAVLINK_MSG_ID_PARAM_REQUEST_LIST:
            {
                ...
                if ((sys != OSD_SYS_ID) || (comp != MAV_COMP_ID_OSD)) break;
            ...
            case MAVLINK_MSG_ID_PARAM_SET:
            {
                ...
                mavlink_msg_param_value_pack(OSD_SYS_ID, MAV_COMP_ID_OSD, &msg2, param_name, param_value, MAVLINK_TYPE_FLOAT, total_params, idx);
...
void send_param_list()
{
    ...
    mavlink_msg_param_value_pack(OSD_SYS_ID, MAV_COMP_ID_OSD, &msg, param_name, param_value, MAVLINK_TYPE_FLOAT, total_params, param_send_index++);
...
void heartbeat_out(void)
{
    ...
    mavlink_msg_heartbeat_pack(OSD_SYS_ID, MAV_COMP_ID_OSD, &msg, MAV_TYPE_ONBOARD_CONTROLLER, MAV_AUTOPILOT_INVALID,
